...
namespace LibOptimization.Optimization
{
  ...
  public class clsOptDEJADE : absOptimization
  {
    ...
    private List<clsPoint> m_archive = new List<clsPoint>();
    ...
    public override void Init()
    {
      try
      {
        ...
        {
          if (UpperBounds.Length != m_func.NumberOfVariable())
            throw new Exception("UpperBounds.Length is different");
          if (LowerBounds.Length != m_func.NumberOfVariable())
        ...
        for (int i = 0; i <= PopulationSize - 1; i++)
        {
          ...
          // bound check
          var tempPoint = new clsPoint(m_func, array);
          if (UpperBounds != null && LowerBounds != null)
    ...
    public override bool DoIteration(int iteration = 0)
    {
      ...
      for (int iterate = 0; iterate <= iteration; iterate++)
      {
        ...
        for (var i = 0; i <= PopulationSize - 1; i++)
        {
          ...
          // pick pBest
          clsPoint pbest /* TODO Change to default(_) if this is not a reference type */;
          var pBestIndex =
          ...
          // Mutation and Crossover
          var child = new clsPoint(ObjectiveFunction);
