    ...
    public class clsOptDE : absOptimization
    {
        ...
        public override void Init()
        {
            try
            {
                ...
                {
                    if (UpperBounds.Length != this.m_func.NumberOfVariable())
                        throw new Exception("UpperBounds.Length is different");
                    if (LowerBounds.Length != this.m_func.NumberOfVariable())
                ...
                for (int i = 0; i <= this.PopulationSize - 1; i++)
                {
                    ...
                    // bound check
                    var tempPoint = new clsPoint(base.m_func, array);
                    if (UpperBounds != null && LowerBounds != null)
        ...
        public override bool DoIteration(int iteration = 0)
        {
            ...
            for (int iterate = 0; iterate <= iteration; iterate++)
            {
                ...
                for (int i = 0; i <= this.PopulationSize - 1; i++)
                {
                    ...
                    clsPoint p1 = this.m_parents[randIndex[0]];
                    clsPoint p2 = this.m_parents[randIndex[1]];
                    clsPoint p3 = this.m_parents[randIndex[2]];
                    clsPoint p4 = this.m_parents[randIndex[3]];
                    clsPoint p5 = this.m_parents[randIndex[4]];

                    // Mutation and Crossover
                    var child = new clsPoint(this.m_func);
                    var j = this.m_rand.Next() % this.m_func.NumberOfVariable();
                    var D = this.m_func.NumberOfVariable() - 1;
                    ...
                    switch (this.DEStrategy)
                    {
                        case EnumDEStrategyType.DE_best_1_bin:
                            {
                                // DE/best/1/bin
                                for (var k = 0; k <= this.m_func.NumberOfVariable() - 1; k++)
                                {
                                    if (this.m_rand.NextDouble() < this.CrossOverRatio || k == D)
                                        child[j] = best[j] + this.F * (p1[j] - p2[j]);
                                    else
                                        child[j] = xi[k];
                                    j = (j + 1) % this.m_func.NumberOfVariable(); // next
                                }
                    ...
                    // Survive
                    if (child.Eval < this.m_parents[i].Eval)
