...
namespace LibOptimization.Optimization
{
    ...
    public class clsOptRealGABLX : absOptimization
    {
        ...
        public clsOptRealGABLX(absObjectiveFunction ai_func)
        {
            ...
            this.PopulationSize = this.m_func.NumberOfVariable() * 50;
            this.ChildrenSize = this.m_func.NumberOfVariable() * 20;
        }
        ...
        public override bool DoIteration(int iteration = 0)
        {
            ...
            for (int iterate = 0; iterate <= iteration; iterate++)
            {
                ...
                // cross over
                List<clsPoint> children = new List<clsPoint>();
                for (int numChild = 0; numChild <= this.ChildrenSize - 1; numChild++)
                {
                    children.Add(new clsPoint(this.m_func));
