...
namespace Util
{
  ...
  public class clsUtil
  {
    ...
    /// Cauchy Distribution
    /// </summary>
    /// <param name="location">default:0</param>
    /// <param name="scale">default:1</param>
    /// <returns></returns>
    /// <remarks>
    /// https://en.wikipedia.org/wiki/Cauchy_distribution
    /// </remarks>
    public static double CauchyRand(double location = 0, double scale = 1)
    {
      return location + scale * Math.Tan(
        Math.PI * (XorShiftPRNGSingleton.GetInstance().NextDouble() - 0.5));
    }
    ...
    /// <returns></returns>
    public static double[] GenRandomPosition(AbsObjectiveFunction func,
      double[] initPosition, double lower, double upper)
    {
      var result = new double[func.Dimension()];
      
      if (initPosition != null && initPosition.Length == func.Dimension())
      {
      ...
      else
      {
        ...
        for (var j = 0; j < func.Dimension(); j++)
        {
      ...
      double[] UpperBounds)
    {
      if (UpperBounds != null && LowerBounds != null)
      {
        for (var ii = 0; ii <= temp.Count - 1; ii++)
        {
          ...
          if (temp[ii] > lower) temp[ii] = lower;
          else if (temp[ii] < upper) temp[ii] = upper;
        }
      }
