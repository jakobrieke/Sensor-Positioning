...
//  }
  ...
  public static void TestGenRandomPosition()
  {
    ...
    for (var i = 0; i < 100; i++)
    {
      ...
      foreach (var d in p)
      {
        Assert.IsTrue(d > -5 && d < 5, "Generated value is outside bounds");
      }
    }
  }
...
//    Console.WriteLine(obj.F(new List<double>(){10, 10, 10}));
    ...
    foreach (var opt in optimizers)
    {
      try
      {
        ...
        if (opt.IterationCount != 0)
          Assert.Fail("fail {0} : 2nd try init iteration count",
            opt.GetType().Name);
  ...
  public static void TestInitialPosition()
  {
    ...
    foreach (var opt in optimizers)
    {
      try
      {
        ...
        clsEasyMatrix resultMat = clsUtil.ToConvertMat(opt.Results);
        clsEasyVector g = new clsEasyVector();
        ...
        if (Math.Abs(range - g[0]) > 10 || Math.Abs(range - g[1]) > 10)
          Assert.Fail("fail {0}", opt.GetType().Name);
      }
      catch (Exception ex)
      {
  ...
  public static void TestOptimizationDEWithBound()
  {
    ...
    else Assert.Fail("fail Eval {0}", opt.Result.Eval);
    ...
    else Assert.Fail("fail Result {0} {1}", opt.Result[0], opt.Result[1]);
  ...
  public static void TestIterationCount()
  {
    ...
    // all iteration
    {
      ...
      foreach (var opt in optimizers)
      {
        ...
        if (opt.IterationCount != opt.Iteration)
          Assert.Fail("{0} : IterationCount : {1}",
            opt.GetType().Name, opt.IterationCount);
    ...
    // per 1
    {
      ...
      foreach (var opt in optimizers)
      {
        ...
        if (opt.IterationCount != opt.Iteration)
          Assert.Fail("Fail! {0} : IterationCount : {1}", opt.GetType().Name,
            opt.IterationCount);
        else
          Console.WriteLine("Success : {0}", opt.GetType().Name);
      }
