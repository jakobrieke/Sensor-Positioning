...
namespace LoMath
{
    ...
    public class clsMathUtil
    {
        ...
        /// <returns></returns>
        public static LoMatrix CreateRandomSymmetricMatrix(int size, int rngSeed = 123456)
        {
            var rng = new System.Random(rngSeed);
            var matTemp = new LoMatrix(size);
        ...
        /// <param name="rngSeed"></param>
        /// <returns></returns>
        public static LoMatrix CreateRandomASymmetricMatrix(int size, int rngSeed = 123456)
        {
            var rng = new System.Random(rngSeed);
            var matTemp = new LoMatrix(size);
            for (int i = 0; i <= matTemp.Count - 1; i++)
        ...
        public static bool IsCheckEigen(int dimNum, int seed = 123456)
        {
            ...
            LoMatrix retM = null/* TODO Change to default(_) if this is not a reference type */;
            LoVector retV = null/* TODO Change to default(_) if this is not a reference type */;
            LoMatrix suspend = null/* TODO Change to default(_) if this is not a reference type */;
            sw.Start();
            while (LoMatrix.Eigen(ref mat2, ref retV, ref retM, SuspendMat: suspend) == false)
            {
        ...
        /// <returns></returns>
        public static bool IsNearyEqualMatrix(LoMatrix matA, LoMatrix matB, double eps = 0.00000001)
        {
            try
            {
                for (int i = 0; i <= matA.RowCount - 1; i++)
                {
                    for (int j = 0; j <= matA.ColCount - 1; j++)
                    {
                        ...
                        var diff = System.Math.Abs(tempValA) - System.Math.Abs(tempValB);
                        if (diff > eps)
                            return false;
                    }
                }
